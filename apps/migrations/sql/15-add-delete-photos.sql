...
CREATE OR REPLACE FUNCTION generate_collection_deleted()
RETURNS VARCHAR(255) AS $$
DECLARE
    prefix VARCHAR(255) := 'deleted-collection-';
    count_id INTEGER;
    new_id VARCHAR(255);
BEGIN
    SELECT COUNT(*) INTO count_id FROM photos.collection_deleted;
...
CREATE TABLE IF NOT EXISTS photos.collection_deleted (
...
CREATE TRIGGER update_collection_timestamp_d
BEFORE UPDATE ON photos.collection_deleted
FOR EACH ROW
...
RETURNS TRIGGER AS $$
BEGIN
    NEW.id := generate_collection_deleted();
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEFORE INSERT ON photos.collection_deleted
FOR EACH ROW
